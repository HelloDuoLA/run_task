...
class camera_detect:
    ...
    def __init__(self, camera_id, marker_size, mtx, dist, arm, tool_len=174, camera_len=70):
        ...
        #                       [0, 0, 0, 1]])
        if arm == 0:
            self.origin_anglesL_vertical = [-83.7, -5.91, -123.05, 123.27, 79.87, 63.91]  # 设置左臂观察点
            self.origin_anglesL_horizontal = [-42.88, 6.93, -87.77, 85.63, 47.29, 53.57]
            ...
            self.origin_anglesL_vertical = [83.7, -5.91, -123.05, -123.27, 90, -63.91]  # 设置右臂观察点
            self.origin_anglesL_horizontal = [42.88, 6.93, -87.77, -85.63, 47.29, -53.57]
            # self.origin_anglesL_vertical = [29.06, 22.62, -99.28, -79.26, 107.03, -21.92]  # 设置右臂观察点
            # self.origin_anglesL_horizontal = [65.15, 6.68, -128.86, -128.55, 86.86, -32.35]
    ...
    def vision_trace(self, mode, ml):
        ...
        if mode == 0:   #水平面抓取
            ...
            time.sleep(1)
            ml.send_base_coords(target_coords, 30)  # 机械臂移动到二维码上方
            self.waitl(ml)  # 等待机械臂运动结束
            ml.send_base_coord(3, target_coords[2] - 30, 30)  # 机械臂沿z轴向下移动
            self.waitl(ml)  # 等待机械臂运动结束
            ml.set_gripper_value(20, 100)  # 闭合夹爪
            time.sleep(1)  # 等待夹爪闭合
            ml.send_base_coord(3, target_coords[2] + 30, 30)  # 抬起夹爪
            self.waitl(ml)
            ml.set_gripper_value(100, 100)  # 打开夹爪
        elif mode == 1:     #竖直面抓取
            ...
            ml.set_gripper_value(100, 100)  # 打开夹爪
            time.sleep(1)
            ml.send_base_coords(target_coords, 30)  # 机械臂移动到二维码前方
            self.waitl(ml)  # 等待机械臂运动结束
            ml.send_base_coord(1, target_coords[0] + 30, 30)  # 机械臂沿x轴向前移动
            self.waitl(ml)  # 等待机械臂运动结束
            ml.set_gripper_value(20, 100)  # 闭合夹爪
            time.sleep(1)  # 等待夹爪闭合
            ml.send_base_coord(1, target_coords[0] - 30, 30)  # 收回夹爪
            self.waitl(ml)
            ml.set_gripper_value(100, 100)  # 打开夹爪
...
if __name__ == "__main__":
    ...
    # ml_obj = camera_detect(3, 32, mtx, dist, 0)
    # ml_obj.vision_trace(1, ml)
    # 3 左摄像头  6右
    mr_obj = camera_detect(6, 32, mtx, dist, 1)
    mr_obj.vision_trace(1, mr)
