...
def voice_to_json(APPID, APIKey, APISecret, messages, mic_index):
    ...
    while recognizer.is_recognizing:
        time.sleep(1)
...
def main():
    ...
    initial_prompt = (
        ...
        "所以有时候可能识别到的文字不太准确，譬如C酷果冻可能识别成西酷果冻等等相近的读音，"
        "所以你要灵活变通一些，即使文字不一样但相近的发音也可以视为是与预设的物品是同一个种类的，"
        "譬如西酷果冻也可以视为是C酷果冻，按照C酷果冻的格式创建json对象。"
        ...
        "机器人要先搬运零食再搬运饮料，而且一次只能运输同一张桌子上的零食或者饮料，因此涉及不同桌子上的零食和饮料时"
        "，要先根据零食分成不同桌子上的订单，再根据饮料分成不同桌子上的订单，比如我的指令是："
        "拿一个C酷果冻和一杯咖啡到左边桌子上，拿一个伊利每益添乳酸菌到右边桌子上，你要生成如下的json内容："
        "{\n"
        "    \"order_operation\": 0,\n"
        "    \"table_id\": 1,\n"
        "    \"snacks\": {\n"
        ...
        "    }\n"
        "}\n"
        "要生成这样三个订单，第一个是搬运左边桌子上的零食，第二个是搬运右边桌子上的零食，"
        "第三个是搬运左边桌子上的饮料，不要将同一张桌子上的饮料和零食合并到同一个订单中"
        "，因为我要先搬运零食再搬运饮料，零食和饮料一定要分开搬运，所以一定要分开为不同的订单。"
        "当输入指令是雪碧、可乐、奶茶等其他不是咖啡的饮品时，你只需要作出以下回答："
        "不好意思，没有（输入指令的物品），饮品只有咖啡可供选择。回答到此为止，不需要再生成json内容了。"
        "不要生成其他任何无关json对象的文字内容，只需要生成上述的json内容,不需要任何有关生成内容的解释,"
        "我就是要你尽可能快的生成出json内容，那些多余的废话解释我一个也不需要！"
        "以上内容都是我给你预设的内容，无需你作任何回答，接下来我跟你说指令时你再生成回答，"
        "总之就是上面这段话只是为了让你知道我需要你做什么事，"
        "我只需要尽可能快的初始化好整个系统，所以你无需作任何浪费时间的回答！"
    )
